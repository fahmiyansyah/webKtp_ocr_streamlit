...
def read_ktp(ktp_path):
    ...
    for line in result.split("\n"):
        if "NIK" in line:
            ...
            if nik_match:
                nik_value = nik_match.group(1)
            # Hapus karakter yang tidak valid dari NIK
                nik_value = re.sub(r'[^\d]', '', nik_value)
                data['nik'] = nik_value
                current_field = 'nik'
        elif "Nama" in line or "kama" in line:
        # Jika kata "kama" terdeteksi, ganti dengan "Nama"
            line = line.replace("kama", "Nama")
            data['nama'] = line.replace("Nama", "").strip()
            current_field = 'nama'
        elif "Tempat/Tgl Lahir" in line or "Tempat/Tg! Lahir" in line or "Tampat/Tgilahir" in line:
        # Jika kata "kama" terdeteksi, ganti dengan "Nama"
            line = line.replace("Tempat/Tg! Lahir", "Tempat/Tgl Lahir").replace("Tampat/Tgilahir", "Tempat/Tgl Lahir")
            data['tempat_tanggal_lahir'] = line.replace("Tempat/Tgl Lahir", "").strip()
            current_field = 'tempat_tanggal_lahir'
        elif "Alamat" in line or "Aiamat" in line:
        # Jika kata "Aiamat" terdeteksi, ganti dengan "Alamat"
            line = line.replace("Aiamat", "Alamat")
        ...
        elif "Jens Kelamin" in line:
            data['jenis_kelamin'] = line.replace("Jens Kelamin", "").strip()
            current_field = 'jenis_kelamin'
            ...
            data['berlaku_hingga'] = line.replace("Berlaku Hingga", "").strip()
            current_field = 'berlaku_hingga'
        elif current_field:
